...
class SqlSyncTest extends CommandUnishTestCase
{
    ...
    public function testSimulatedSqlSync()
    {
        ...
        $this->assertContains("Simulating backend invoke: ssh -o PasswordAuthentication=whatever www-admin@server.isp.simulated '/path/to/drush --backend=2 --strict=0$expectedAliasPath --root=__SUT__/web --uri=remote sql-dump --no-interaction --gzip --result-file", $output);
        ...
        $this->assertContains("Simulating backend invoke: ssh -o PasswordAuthentication=whatever www-admin@server.isp.simulated '/path/to/drush --backend=2 --strict=0$expectedAliasPath --root=__SUT__/web --uri=remote sql-query --no-interaction --file=/tmp/dump.tgz --file-delete", $output);
