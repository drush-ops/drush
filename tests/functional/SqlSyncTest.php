...
class SqlSyncTest extends CommandUnishTestCase
{
    public function testSimulatedSqlSync(): void
    {
        ...
        $this->assertStringContainsString("[notice] Simulating: __DIR__/drush.php core:rsync @synctest.remote:/simulated/path/to/dump.tgz @synctest.local:__SANDBOX__/tmp/dump.tgz --yes --no-interaction --strict=0 --uri=local -- --remove-source-files", $output);
        ...
        $this->assertStringContainsString("[notice] Simulating: __DIR__/drush.php core:rsync @synctest.local:/simulated/path/to/dump.tgz @synctest.remote:/tmp/dump.tgz --yes --no-interaction --strict=0 --uri=local -- --remove-source-files", $output);
