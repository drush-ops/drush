...
class SqlSyncTest extends CommandUnishTestCase
{
    ...
    public function testSimulatedSqlSync()
    {
        ...
        $this->assertContains("[notice] Simulating: __DIR__/drush core-rsync @synctest.remote:/simulated/path/to/dump.tgz @synctest.local:__DIR__/sandbox/tmp/dump.tgz --uri=local --root=__DIR__/sut -- --remove-source-files", $output);
        $this->assertContains("[notice] Simulating: __DIR__/drush sql-query --no-interaction --strict=0 --file=__DIR__/sandbox/tmp/dump.tgz --file-delete --uri=local --root=__DIR__/sut", $output);
