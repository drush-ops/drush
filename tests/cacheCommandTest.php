...
class cacheCommandCase extends CommandUnishTestCase {
  ...
  function testCacheSet() {
    ...
    $exec = sprintf('%s cache-set %s %s my_cache_id - %s CACHE_PERMANENT --input-format=json --cache-get 2>%s', UNISH_DRUSH_PHP, "--root=" . self::escapeshellarg($options['root']), '--uri=' . $options['uri'], $bin, $this->bit_bucket());
