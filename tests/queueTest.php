...
class QueueCase extends CommandUnishTestCase {
  ...
  function testQueue() {
    ...
    $this->drush('pm-enable', array('aggregator'));
    ...
    $this->drush('user-create', array('example'), ['password' => 'password', 'mail' => "example@example.com"]);
    $this->drush('php-script', array('queue_script'), array('script-path' => __DIR__ . '/resources'));
    $this->drush('queue-list', array(), array('format' => 'csv'));
    $output = $this->getOutputAsList();
    $this->assertEquals(str_replace('%items', 1, $expected), array_pop($output), 'Item was successfully added to the queue.');

    $this->drush('queue-run', array('aggregator_feeds'));
    $this->drush('queue-list', array(), array('format' => 'csv'));
    $output = $this->getOutputAsList();
  ...
  public function testQueueDelete() {
    ...
    $this->drush('queue-delete', array('aggregator_feeds'));
  ...
  public function testRequeueException() {
    ...
    // RequeueException.
    $this->drush('pm-enable', array('woot'), [], NULL, NULL, self::EXIT_SUCCESS);

    // Add an item to the queue.
    $this->drush('php-script', array('requeue_script'), array('script-path' => __DIR__ . '/resources'));
    ...
    // Process the queue.
    $this->drush('queue-run', array('woot_requeue_exception'));
