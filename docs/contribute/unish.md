...
Drush provides test traits that may be used to test your own Drush extensions. Adding the traits varies slightly depending how you package your Drush extension.
  ...
  - An extension that ships inside a contributed module - [DevelCommandsTest](https://git.drupalcode.org/project/devel/-/blob/5.x/tests/src/Functional/DevelCommandsTest.php) for an example. More examples are [SchedulerDrushTest](https://git.drupalcode.org/project/scheduler/-/blob/2.x/tests/src/Functional/SchedulerDrushTest.php?ref_type=heads) and [Views Bulk Operations](https://git.drupalcode.org/project/views_bulk_operations/-/blob/4.3.x/tests/src/Functional/DrushCommandsTest.php?ref_type=heads). Remember to add `drush/drush` to your module's composer.json (`require-dev` section).
