...
- If you need to ask the user a question, or print non-object content, build a style object via `$io = new DrushStyle($input, $output);`
- The main methods for gathering user input are `$io->select()` and `$io()->confirm()`. See [Laravel Prompts](https://laravel.com/docs/10.x/prompts) for other methods.
