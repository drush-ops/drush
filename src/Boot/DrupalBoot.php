...
abstract class DrupalBoot extends BaseBoot
{
    ...
    public function bootstrapDrupalRoot(BootstrapManager $manager)
    {
        $drupal_root = $manager->getRoot();
        chdir($drupal_root);
        $this->logger->log(LogLevel::INFO, dt("Change working directory to !drupal_root", ['!drupal_root' => $drupal_root]));
        ...
        $this->logger->log(LogLevel::INFO, dt("Initialized Drupal !version root directory at !drupal_root", ["!version" => Drush::bootstrap()->getVersion($drupal_root), '!drupal_root' => $drupal_root]));
    }
    ...
    public function bootstrapDrupalDatabase(BootstrapManager $manager)
    {
        ...
        $this->logger->log(LogLevel::INFO, dt("Successfully connected to the Drupal database."));
    }
