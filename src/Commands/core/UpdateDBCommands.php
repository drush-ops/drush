...
class UpdateDBCommands extends DrushCommands {
   ...
   */
  function updateDoOne($module, $number, $dependency_map,  &$context) {
   ...
   */
  function updateEntityDefinitions(&$context) {
    try {
...
// Copy of protected \Drupal\system\Controller\DbUpdateController::getModuleUpdates.
   ...
   */
  function updateFinished($success, $results, $operations) {
  ...
  function entityUpdatesMain() {
    ...
    if (!empty($change_summary)) {
      ...
      $batch += array(
        'title' => 'Updating',
        'init_message' => 'Starti ng updates',
        'error_message' => 'An unrecoverable error has occurred. You can find the error message below. It is advised to copy it to the clipboard for reference.',
